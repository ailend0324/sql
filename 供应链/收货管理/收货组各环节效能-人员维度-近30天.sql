WITH
    ...
    agg_unpack AS (
        SELECT
            ...
            CASE
                WHEN ftype = '回收' THEN IF(CONCAT(ftype, '拆包', Fcategory_name) IS NULL, CONCAT(ftype, '拆包'), CONCAT(ftype, '拆包', Fcategory_name))
        ...
        WHERE fseries_number IS NOT NULL
          AND funpack_time IS NOT NULL
          AND Funpack_user IS NOT NULL
        GROUP BY 1, 2, 3, 4, 5
    ),

    -- 02 验机配件收货（parcel）
    agg_parts_receive AS (
        SELECT
    ...
    agg_receive AS (
        SELECT
            ...
            END AS fwarehouse,
            CASE
                WHEN TO_DATE(freceive_time) = '2024-05-29' AND freceive_user = '朱小洋' AND LEFT(fseries_number, 2) = '01' THEN '李伟豪'
            ...
            CASE
                ...
                WHEN ftype = '验机' AND fcategory_name IN ('平板', '平板电脑') THEN CONCAT(ftype, '收货', '平板')
                WHEN ftype = '回收' AND fcategory_name IN ('CPU', '内存条', '显卡', '智能手写笔', '游戏机', '电子书', '电脑主板', '固态硬盘', '显示器') THEN CONCAT(ftype, '收货', '电脑配件')
                WHEN ftype = '回收' AND fcategory_name IS NOT NULL AND fcategory_name NOT IN ('手机', '平板', '平板电脑', '笔记本', '笔记本电脑', '耳机', '配件', '充电套装', '智能手表', '智能手环') THEN CONCAT(ftype, '收货', '相机及其它')
                WHEN ftype = '回收' AND fcategory_name IS NULL THEN CONCAT(ftype, '收货', '相机及其它')
                WHEN ftype = '回收' THEN IF(CONCAT(ftype, '收货', Fcategory_name) IS NULL, CONCAT(ftype, '收货'), CONCAT(ftype, '收货', Fcategory_name))
    ...
    agg_main_photo AS (
        SELECT
            ...
            END AS fwarehouse,
            CASE
                WHEN TO_DATE(fmain_photo_time) = '2024-02-03' AND fphoto_name = '杨泽文' THEN '朱小洋'
        ...
        FROM dws.dws_instock_details
        WHERE fseries_number IS NOT NULL
          AND fmain_photo_time IS NOT NULL
    ...
    agg_defect_photo AS (
        ...
        FROM dws.dws_instock_details
        WHERE fseries_number IS NOT NULL
          AND fdefect_photo_time IS NOT NULL
    ...
    agg_plus_print AS (
        SELECT
            ...
            END AS fwarehouse,
            CASE
                WHEN TO_DATE(fplus_print_time) = '2024-06-05' AND fplus_printer = '吴依卓' THEN NULL
                ELSE fplus_printer
            END AS operator,
            '寄卖打印' AS ftype,
            COUNT(fseries_number) AS num,
        ...
        FROM dws.dws_instock_details
        WHERE fseries_number IS NOT NULL
          AND fplus_print_time IS NOT NULL
    ...
    agg_yhb_out AS (
        SELECT
            ...
            CASE
                WHEN LEFT(fhost_barcode, 2) IN ('02') OR RIGHT(LEFT(fhost_barcode, 6), 2) = '16' THEN '杭州'
                ELSE '深圳'
            END AS fwarehouse,
    ...
    agg_allot_add AS (
        SELECT
            ...
            CASE
                WHEN LEFT(fbar_code, 2) IN ('02') OR RIGHT(LEFT(fbar_code, 6), 2) = '16' THEN '杭州'
                ELSE '深圳'
            END AS fwarehouse,
    ...
    agg_allot_update AS (
        ...
        FROM allot
        WHERE fupdate_time IS NOT NULL
          AND fbar_code IS NOT NULL
    ...
    agg_auto_det AS (
        SELECT
            ...
            CASE
                WHEN LEFT(fserial_number, 2) IN ('02') OR RIGHT(LEFT(fserial_number, 6), 2) = '16' THEN '杭州'
                ELSE '深圳'
            END AS fwarehouse,
            CASE
                WHEN TO_DATE(fcreate_time) = '2023-10-16' AND Fbind_real_name = '刘俊' THEN '周利'
    ...
    agg_seal_bag AS (
        SELECT
            ...
            END AS fwarehouse,
            CASE
                WHEN TO_DATE(FROM_UNIXTIME(fadd_time)) = '2024-06-05' AND freal_name = '吴依卓' THEN NULL
                ELSE freal_name
            END AS operator,
            '装密封袋' AS ftype,
            COUNT(fseries_number) AS num,
        ...
        FROM seal_bag
        WHERE fseries_number IS NOT NULL
          AND FROM_UNIXTIME(fadd_time) IS NOT NULL
          AND freal_name IS NOT NULL
        GROUP BY 1, 2, 3, 4, 5
    ),

    unioned AS (
        SELECT * FROM agg_unpack
        UNION ALL SELECT * FROM agg_parts_receive
        UNION ALL SELECT * FROM agg_receive
        UNION ALL SELECT * FROM agg_main_photo
        UNION ALL SELECT * FROM agg_defect_photo
        UNION ALL SELECT * FROM agg_tamper
        UNION ALL SELECT * FROM agg_plus_print
        UNION ALL SELECT * FROM agg_yhb_out
        UNION ALL SELECT * FROM agg_allot_add
        UNION ALL SELECT * FROM agg_allot_update
        UNION ALL SELECT * FROM agg_seal_bag
        UNION ALL SELECT
            ftimeby,
            fchannel AS channel,
            fwarehouse,
            freal_name AS operator,
            ftype,
            num,
            `加班前数量`,
            `加班后数量`
...
SELECT 
    ...
    `加班后数量`
FROM unioned
WHERE TO_DATE(ftimeby) >= TO_DATE(DATE_SUB(FROM_UNIXTIME(UNIX_TIMESTAMP()), 30))