...
select                                           --小米商城新机退回物流信息
    l.Freturn_time,
    null as fstock_order_sn,
    null as fserial_number,
    case  l.Fdeliver_order_type 
	when 1 then "小米商城新机退回"
	when 2 then "小米商城赠品退回"
	else cast( l.Fdeliver_order_type  as string) end
	as ftype,
    upper(l.Freturn_express_no) as fexpress_reality_sn,
    "小米上门以旧换新"  as "业务类型",
    '自有'  as "流量来源",
    null as fold_series_number,
    c.fchannel_name,
    o.forder_num as forder_num,
    l.Forder_id as forder_id,
    null fclass_name,
    null  as forder_platform,
    null as fsales_order_num,
    case o.fsupply_partner
	when 2 then "小站(自营)"
	when 3 then "小站(加盟)" 
	else cast(o.fsupply_partner as string) end 	as "履约方",
    case when left(l.Freturn_express_no,2) like "%JD%" then "京东"
         when left(l.Freturn_express_no,2) like "%SF%" then "顺丰"
         when left(l.Freturn_express_no,3) like "%ZTO%" then "中通"
    else "其它" end as "快递类型"

from   drt.drt_my33310_recycle_t_xiaomi_old4new_logistics l
left join drt.drt_my33310_recycle_t_order o on o.forder_id = l.forder_id
left join drt. drt_my33310_recycle_t_channel c on c.fchannel_id = o.fchannel_id
where l.Fvalid = 1 and l.Freturn_time > '2025-09-01' 
-- and l.Fdeliver_order_type =1
