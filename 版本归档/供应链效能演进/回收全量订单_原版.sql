...
detect as (
    ...
    from (
        ...
        and fis_deleted=0
      	and freport_type=0
    	and fverdict<>"测试单"
        and to_date(fend_time) >=to_date((select default_window from time_config)))t
    where num=1
)
select             --回收订单信息匹配
...
left join 
    ...
    and to_date(fauto_update_time)>=to_date((select default_window from time_config)))t
    where num=1
...
select                                          --寄卖订单
    ...
    from 
        ...
        left join drt.drt_my33310_recycle_t_xy_jimai_plus_order as b on a.forder_id=b.forder_id
        where a.forder_status in (714,815)
        and to_date(a.fauto_update_time)>=to_date((select default_window from time_config))) t
    where num=1
    ) as d on a.forder_id=d.forder_id
    left join drt.drt_my33310_recycle_t_channel as c on a.fchannel_id=c.fchannel_id
    left join drt.drt_my33310_recycle_t_product as e on a.fproduct_id=e.fproduct_id
