...
and to_date(from_unixtime(a.fend_det_time))>=to_date(date_sub(from_unixtime(unix_timestamp()),600)))t
where num=1
),
detect_two as (
select 
...
left join drt.drt_my33310_amcdb_t_user as b on a.fuser_name=b.fusername
where to_date(a.fcreate_time)>=to_date(date_sub(from_unixtime(unix_timestamp()),600))
and fserial_number!=""
...
from (
    ...
    where to_date(a.fend_time)>=to_date(date_sub(from_unixtime(unix_timestamp()),600))
...
and t.Fadd_time>=to_date(date_sub(from_unixtime(unix_timestamp()),600))
)
SELECT 
    a.forder_id,
	a.fxy_order_id,
	...
	a.fxy_channel,
	a.fsub_channel,
	if(a.fhsb_product_name is null or a.fhsb_product_name="",c.fproduct_name,a.fhsb_product_name) as fhsb_product_name,
	a.fend_status,
	a.fhost_barcode,
	a.fparts_barcode,
	a.flogistics_number,
	...
	a.freceiver_address,
	a.freceive_time,
	if(c.fend_time>a.fdetect_put_time or c.fend_time is null,a.fdetect_time,c.fend_time)as fdetect_time,
	if(i.freal_name is not null,i.freal_name,a.fengineer_real_name) as fengineer_real_name,
	a.fphoto_name,
	a.fphoto_time,
	a.fshooting_time,
    ...
    f.fstock_in_time,
    f.fsale_out_time,
    if(f.fstock_in_time is not null and f.fsale_out_time is not null,(unix_timestamp(f.fsale_out_time,'yyyy-MM-dd HH:mm:ss')-unix_timestamp(f.fstock_in_time,'yyyy-MM-dd HH:mm:ss'))/3600/24,null) as fsale_time,
...
where forder_time>=to_date(date_sub(from_unixtime(unix_timestamp()),600))
