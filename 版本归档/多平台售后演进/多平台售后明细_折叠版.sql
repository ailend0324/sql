-- ============================================
-- 竞拍售后明细（含鱼市 B2B 渠道） - 可折叠版本
-- 折叠标记：--region / --endregion
-- ============================================

--region A. 公共“检测/售后检测” CTE
WITH
--region detect（原口径初检）
detect AS (
  SELECT *
...
--region after_sale_detect（售后场景-检测首条）
...
--endregion after_sale_detect

--region detect_one / detect_two / detect_three / pingmu / four
detect_one AS (
  SELECT
...
--endregion detect_one / detect_two / detect_three / pingmu / four
--endregion A. 公共“检测/售后检测” CTE


--region B. 竞拍（自有平台）与二次销售 CTE
--region jp_sale（近365天 + 2023 历史）
jp_sale AS (
  SELECT *
  FROM (
    SELECT
...
--endregion jp_first_sale

--region jp_second_sale（二次销售明细汇总）
jp_second_sale AS (
  SELECT *
  FROM (
    SELECT
...
--endregion jp_second_sale
--endregion B. 竞拍（自有平台）与二次销售 CTE


--region C. 采货侠（平台=6）CTE
--region after_sale（采货侠售后主表）
after_sale AS (
  SELECT *
  FROM (
    SELECT
...
--endregion after_sale

--region caihuoxia_sale / caihuoxia_after_sale / first / second
caihuoxia_sale AS (
  SELECT *
  FROM (
    SELECT
...
--endregion caihuoxia_sale / caihuoxia_after_sale / first / second
--endregion C. 采货侠（平台=6）CTE


--region D. 鱼市 B2B（平台=5）CTE
--region b2b_sale / b2b_detect / b2b_after_sale / b2b_first_sale
,b2b_sale AS (
  SELECT *
  FROM (
    SELECT
...
--endregion b2b_sale / b2b_detect / b2b_after_sale / b2b_first_sale

--region b2b_detect_two / b2b_detect_three / pingmu / b2b_second_sale
b2b_detect_two AS (
  SELECT
    UPPER(fserial_number) AS fserial_number,
    freal_name            AS fdetect_two_name,
...
--endregion b2b_detect_two / b2b_detect_three / pingmu / b2b_second_sale
--endregion D. 鱼市 B2B（平台=5）CTE


--region E. 最终结果集：自有平台 ∪ 采货侠 ∪ B2B
SELECT
  a.fstart_time,
  a.fseries_number,
  a.fclass_name,
...
--endregion E. 最终结果集：自有平台 ∪ 采货侠 ∪ B2B
;