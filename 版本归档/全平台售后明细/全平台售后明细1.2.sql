-- ========================================
-- 全平台售后明细1.2版本：优化历史表使用
-- 优化内容：
-- 1. order_base_jingpai分支：移除历史表（贡献率0%）
-- 2. dws_th_order_detail分支：保留历史表（贡献率100%）
-- 预期收益：减少50%的历史表查询，提升性能
-- 数据影响：无影响（order_base_jingpai分支历史表贡献率0%）
-- ========================================

-- 注：这里只展示 jp_second_sale 部分的优化，完整SQL请参考1.1版本
...
jp_second_sale as (
    -- 分支1：order_base_jingpai（移除历史表，因为贡献率0%）
    select 
        b.fold_fseries_number as fold_fseries_number,  -- 只从现网表获取，历史表贡献率0%
        a.foffer_price, 
        a.Fcity_name, 
        a.Forder_address, 
        a.Freceiver_id, 
    ...
    -- 已移除：left join dws.dws_hs_order_detail_history2018_2022 as c ...
    -- 原因：验证显示该分支历史表贡献率为0%，可以安全移除
    
    union all
    
    -- 分支2：dws_th_order_detail（保留历史表，因为贡献率100%）
    select 
