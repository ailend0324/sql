...
qc_detect as(
        ...
        from (select 
            ...
            -- and (freal_name like "%赖嘉琪%")
             and (freal_name like "%赖嘉琪%" or freal_name like "%李伟雪%" or freal_name like "%王封敏%" or freal_name like "%王若桂%" or freal_name like "%范嘉庆%" or freal_name like "%陈斌%" or freal_name like "%陈梓琦%" or freal_name like "%周雨%" or freal_name like "%郑君豪%" )
            and to_date(fend_time) >=to_date(date_sub(from_unixtime(unix_timestamp()),180)))a
        left join (
          select 
            fdetect_record_id,
