...
and to_date(from_unixtime(a.fend_det_time)) between to_date(date_sub(from_unixtime(unix_timestamp()),60)) and to_date(date_sub(from_unixtime(unix_timestamp()),1))
)t
where num=1
),
detect_two as (
...
left join drt.drt_my33310_amcdb_t_user as b on a.fuser_name=b.fusername
where to_date(a.fcreate_time)between to_date(date_sub(from_unixtime(unix_timestamp()),60)) and to_date(date_sub(from_unixtime(unix_timestamp()),1))
and fserial_number!=""
...
from (
    ...
    where to_date(a.fend_time)between to_date(date_sub(from_unixtime(unix_timestamp()),60)) and to_date(date_sub(from_unixtime(unix_timestamp()),1))
...
select 
    to_date(fdetect_one_time),
    hour(fdetect_one_time),
    ...
    "模块一" as fdet_type,
    case when right(left(fserial_number,6),2)="16" or left(fserial_number,3)="020" then "杭州" else "深圳" end as fwarehouse,
    fdetect_one_name as freal_name,
    case when fbrand_name="苹果" then "苹果" else "安卓" end as fbrand,
    count(distinct fserial_number)
from detect_one
group by 1,2,3,4,5,6,7
union all
select 
    to_date(fdetect_two_time),
    hour(fdetect_two_time),
    case when left(fserial_number,1)="0" then "验机" 
         when left(fserial_number,2)='BM' then "寄卖" 
    ...
    "模块二" as fdet_type,
    case when right(left(fserial_number,6),2)="16" or left(fserial_number,3)="020" then "杭州" else "深圳" end as fwarehouse,
    fdetect_two_name as freal_name,
...
union all
select 
    to_date(fdetect_three_time),
    hour(fdetect_three_time),
    case when left(fserial_number,1)="0" then "验机" 
         when left(fserial_number,2)='BM' then "寄卖" 
    ...
    "模块三" as fdet_type,
    case when right(left(fserial_number,6),2)="16" or left(fserial_number,3)="020" then "杭州" else "深圳" end as fwarehouse,
    fdetect_three_name as freal_name,
    case when fbrand_name="苹果" then "苹果" else "安卓" end as fbrand,
    count(distinct fserial_number)
from detect_three
group by 1,2,3,4,5,6,7
union all
select 
    to_date(fdetect_four_time),
    hour(fdetect_four_time),
    case when left(fserial_number,1)="0" then "验机" 
         when left(fserial_number,2)='BM' then "寄卖" 
    ...
    "模块四" as fdet_type,
    case when right(left(fserial_number,6),2)="16" or left(fserial_number,3)="020" then "杭州" else "深圳" end as fwarehouse,
    fdetect_four_name as freal_name,
    case when fbrand_name="苹果" then "苹果" else "安卓" end as fbrand,
    count(distinct fserial_number)
from detect_four
group by 1,2,3,4,5,6,7
