...
left join (
  ...
  as b on a.fdetect_record_id=b.fdetect_record_id
where a.num=1
...
qc_detect as(
        ...
        from (select 
            ...
            -- and (freal_name like "%赖嘉琪%")
             and (freal_name like "%赖嘉琪%" or freal_name like "%李伟雪%" or freal_name like "%王封敏%" or freal_name like "%王若桂%" or freal_name like "%范嘉庆%" or freal_name like "%陈斌%" or freal_name like "%陈梓琦%" or freal_name like "%周雨%" or freal_name like "%郑君豪%"  or freal_name like "%叶思宁%")
            and to_date(fend_time) >=to_date(date_sub(from_unixtime(unix_timestamp()),180)))a
        left join (
          select 
            fdetect_record_id,
            ...
            field_source
 		from dwd.dwd_detect_back_detection_issue_and_answer_v2
		where ds>=to_date(date_sub(from_unixtime(unix_timestamp()),180)))  as b on a.fdetect_record_id=b.fdetect_record_id
        where a.num=1
        and b.field_source='fdet_norm_snapshot'
),
detect_three as (
...
select 
    ...
    case when a.fwarehouse_code='12' then "东莞仓" 
    	 when right(left(a.fserial_number,6),2)="16" or left(a.fserial_number,3)="020" then "杭州仓"  else "深圳仓" end as fwarehouse_code
...
and (a.fissue_name like "%外观%" or a.fissue_name like "%屏幕%" or a.fissue_name like "%边框%" or a.fissue_name like "%显示%" or a.fissue_name like "%机身弯曲%" or a.fissue_name like "%折叠屏转轴%" or a.fissue_name like "%折叠屏保护膜%")
