...
-- 时间窗：统一采用"最近 N 天"，默认 400 天（可调），确保所有子查询口径一致
-- 主要来源表：
...
--
-- 优化说明：
--   1. 字段按业务流程重新排序：基础信息 → 检测过程时间 → 人员信息 → 产品信息 → 检测结果标记 → 业务分类 → 绩效相关
--   2. 添加清晰的分组注释，便于理解和维护
--   3. 检测过程时间提前，便于分析检测流程的时间节点

-- 统一时间窗（可调）
with params as (
  select date_sub(from_unixtime(unix_timestamp()), 400) as dt_from
),
...
select 
    ...
    to_date(t.fend_time) as 检测日期,
    t.fend_time as 检测完成时间,
    case when day(t.fend_time)>=25 
         then concat(cast(year(t.fend_time) as string), '年', cast(month(t.fend_time)+1 as string), '月') 
         else concat(cast(year(t.fend_time) as string), '年', cast(month(t.fend_time) as string), '月') end as '绩效年月',
    
    -- ========== 2. 检测过程时间 ==========
    d.fdetect_one_time as 模块1时间,
    e.fdetect_two_time as 模块2时间,
    f.fdetect_three_time as 模块3时间,
    h.fdetect_three_time_pingmu as 屏幕检测时间,
    g.fdetect_four_time as 模块4时间,
    
    -- ========== 3. 人员信息 ==========
    t.freal_name as 检测人员,
    if((to_date(d.fdetect_one_time) between '2024-02-26' and '2024-02-27' or to_date(d.fdetect_one_time)='2024-02-29') and d.fdetect_one_name='陈冬凡','高华铎',
       if(to_date(d.fdetect_one_time) between '2024-02-26' and '2024-02-27' and d.fdetect_one_name='赖云龙','刘海其',d.fdetect_one_name)) as 模块1人员,
    -- 模块2人员优化逻辑：优先检查赖燊，其次曹开放，否则使用原值
    case when d.fdetect_one_name='赖燊' or e.fdetect_two_name='赖燊' or f.fdetect_three_name='赖燊' or g.fdetect_four_name='赖燊' or t.freal_name='赖燊' then '赖燊'
         when d.fdetect_one_name='曹开放' or e.fdetect_two_name='曹开放' then '曹开放'
         else e.fdetect_two_name end as 模块2人员,
    -- 模块3人员优化逻辑：空值回退到主检人员
    case when f.fdetect_three_name is null and d.fdetect_one_time > '2023-01-01' then t.freal_name
         else f.fdetect_three_name end as 模块3人员,
    h.fdetect_three_name_pingmu as 屏幕检测人员,
    -- 模块4人员优化逻辑：空值回退到主检人员
    case when g.fdetect_four_name is null then t.freal_name
         else g.fdetect_four_name end as 模块4人员,
    
    -- ========== 4. 产品信息 ==========
    t.fproduct_name as 品名,
    t.fclass_name as 品类,
    case when t.fbrand_name like '%苹果%' then '苹果' else '安卓' end as fbrand,
    t.fbrand_name as '品牌(原始)',
    case when t.fproduct_name in (
        '三星 Galaxy Fold','三星 Galaxy Fold（5G）','三星 W20（5G）','华为 Mate Xs（5G）','三星 Galaxy Z  Fold2（5G）',
        '三星 W21（5G）','小米 MIX FOLD（5G）','三星 Galaxy Z Fold3（5G）','三星 W22（5G）','荣耀 Magic V（5G）',
        'vivo X Fold（5G）','小米 MIX FOLD2（5G）','三星 Galaxy Z Fold4（5G）','vivo X Fold+（5G）','三星 W23（5G）',
        '荣耀 Magic Vs（5G）','荣耀 Magic Vs 至臻版','华为 Mate X3','华为 Mate X2 典藏版','华为 Mate X3 典藏版',
        'vivo X Fold 2（5G）','谷歌 Pixel Fold','荣耀 Magic V2（5G）','荣耀 Magic V2 至臻版','三星 Galaxy Z Fold5（5G）',
        '小米 MIX FOLD3（5G）','华为 Mate X5','华为 Mate X5 典藏版','三星 W24（5G）','荣耀 Magic Vs 2（5G）',
        'OPPO Find N3（5G）','荣耀 Magic V2 RSR 保时捷设计','vivo X Fold 3','vivo X Fold3 Pro','荣耀 Magic V3',
        '三星 Galaxy Z Fold6（5G）','小米 MIX FOLD 4','荣耀 Magic Vs 3','华为 Mate X2（5G）','华为 Mate X2（4G）',
        '一加 Open','华为 Mate XT 非凡大师','三星 W25','华为 Mate X6','华为 Mate X6 典藏版','OPPO Find N5','vivo X Fold 5'
    ) then '是' else '否' end as '是否折叠屏',
    t.fdetect_price/100 as 检测价格,
    ...
    -- ========== 6. 业务分类 ==========
    -- 业务分类（简化逻辑）
    case when left(t.fserial_number,3) in ('020','010','050') then '验机'
    ...
    case when left(t.fserial_number,3) in ('020','010','050') then '验机' else '回收' end as '业务类型',
    case 
    ...
    -- 项目渠道（简化逻辑）
    case when (b.fproject_name='自有项目' and b.fchannel_name='支付宝小程序') or left(t.fserial_number,3) in ('020','010','050') then '合作项目'
         else b.fproject_name end as 项目,
    case when left(t.fserial_number,3) in ('020','010','050') then '闲鱼验机' else b.fchannel_name end as 渠道,
    case 
    ...
    case when t.fproduct_name like '%公益%' then 0
         when left(t.fserial_number,2) not in ('BM','JM','XZ') and t.Fdet_tpl <> 4 and left(t.fserial_number,3) not in ('020','010','050') then 1 
         else 0 end as '邮寄回收检测量'
from (
