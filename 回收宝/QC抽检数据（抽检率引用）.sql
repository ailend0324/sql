...
from( 
    ...
    and to_date(fend_time)>=to_date(date_sub(from_unixtime(unix_timestamp()),365))  
)a
...
qc_detect as(                           -- 第二步：获取QC抽检数据
        ...
        from (select 
            ...
            and (freal_name like "%赖嘉琪%" or freal_name like "%李伟雪%" or freal_name like "%王封敏%" 
                 ...
                 or freal_name like "%叶思宁%" or freal_name like "%刘凤瑊%" or freal_name like "%马显岩%" 
                 or freal_name like "%赵语放%")
            -- ↑ 只统计这些指定的QC检测员的检测记录
            -- 就像只统计质检部门特定人员的抽检工作
...
where to_date(a.fend_time)>=to_date(date_sub(from_unixtime(unix_timestamp()),365))  -- 最近365天
