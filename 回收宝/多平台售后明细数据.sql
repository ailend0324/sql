...
detect as (       --取最新检测明细数据，取检测人、检测模板（优化版：统一365天窗口）
    select
        a.fcreate_time,
    ...
    inner join (
        ...
        qualify row_number() over(partition by fseries_number order by forder_create_time desc) = 1
    ...
    inner join (
        ...
        qualify row_number() over(partition by freal_name order by fcreate_time desc) = 1
    ...
    -- 优化：使用QUALIFY替代复杂的子查询
    qualify row_number() over(partition by upper(a.fserial_number) order by a.fcreate_time asc) = 1
),
after_sale_detect as (
    ...
    qualify row_number() over(partition by fserial_number order by fend_time asc) = 1
),
jp_sale as (
    select *
    from (
...
jp_first_sale as (  --第一次销售（优化版）
    select *
    from (
        select 
            *,
            row_number() over(partition by fseries_number order by forder_create_time asc) as rn
        from dws.dws_jp_order_detail
        where ftest_show <> 1
        and forder_create_time >= (select start_date_365 from time_constants)
        and forder_platform not in (5,6)
...
after_sale as (
    ...
    where a.fvalid=1
    qualify row_number() over(partition by fsales_series_number order by a.fauto_create_time desc) = 1
),
caihuoxia_sale as (
    select *
    from (
...
caihuoxia_after_sale as (
    ...
    qualify row_number() over(partition by fbusiness_id order by fcreate_time desc) = 1
),
...
detect_one as (
    ...
    where fserial_number != "" and fserial_number is not null
    and to_date(from_unixtime(a.fend_det_time)) >= (select start_date_800 from time_constants)
    qualify row_number() over(partition by upper(a.fserial_number) order by a.fend_det_time desc) = 1
),
detect_two as (
    select
        upper(fserial_number) as fserial_number,
    ...
    qualify row_number() over(partition by upper(a.fserial_number) order by a.fcreate_time desc) = 1
),
detect_three as (
    select
        upper(fserial_number) as fserial_number,
...
b2b_detect as (       -- B2B初检：窗口365天，平台=5（优化版）
    select
        a.fcreate_time,
        upper(a.fserial_number) as fserial_number,
        a.Fdet_tpl,
...
b2b_first_sale as (
    ...
    and forder_create_time >= (select start_date_365 from time_constants)
    and forder_status in (2,3,4,6)
    qualify row_number() over(partition by fseries_number order by forder_create_time asc) = 1
),
b2b_detect_two as (
    select
        upper(fserial_number) as fserial_number,
    ...
    from drt.drt_my33312_detection_t_det_app_record as a
    left join drt.drt_my33310_amcdb_t_user as b on a.fuser_name=b.fusername
    where to_date(a.fcreate_time) >= (select start_date_365 from time_constants)
    and fserial_number != "" and fserial_number is not null
    qualify row_number() over(partition by upper(a.fserial_number) order by a.fcreate_time desc) = 1
),
b2b_detect_three as (
    ...
    from drt.drt_my33312_detection_t_det_task as a
    left join drt.drt_my33312_detection_t_det_task_record as b on a.ftask_id=b.ftask_id
    where to_date(a.fend_time) >= (select start_date_365 from time_constants)
    and b.fdet_sop_task_name like "%外观%"
    qualify row_number() over(partition by upper(a.fserial_number) order by a.fcreate_time desc) = 1
),
...
/*
-- 主要表索引建议：
-- 1. dws.dws_jp_order_detail
CREATE INDEX idx_jp_order_detail_platform_status_time ON dws.dws_jp_order_detail (forder_platform, forder_status, forder_create_time, ftest_show, fmerchant_jp);

-- 2. drt.drt_my33310_detection_t_detect_record  
CREATE INDEX idx_detect_record_serial_time ON drt.drt_my33310_detection_t_detect_record (fserial_number, fend_time, fis_deleted);

-- 3. drt.drt_my33310_recycle_t_after_sales_order_info
CREATE INDEX idx_after_sales_series_time ON drt.drt_my33310_recycle_t_after_sales_order_info (fsales_series_number, fauto_create_time, fvalid);

-- 4. drt.drt_my33306_hsb_sales_t_caihuoxia_after_sales
CREATE INDEX idx_caihuoxia_after_sales_business ON drt.drt_my33306_hsb_sales_t_caihuoxia_after_sales (fbusiness_id, fcreate_time);

-- 5. drt.drt_my33310_amcdb_t_user
CREATE INDEX idx_user_name_position ON drt.drt_my33310_amcdb_t_user (freal_name, Fposition_id, fcreate_time);

-- 6. 检测模块相关表
CREATE INDEX idx_det_app_record_serial_time ON drt.drt_my33312_detection_t_det_app_record (fserial_number, fcreate_time);
CREATE INDEX idx_det_task_serial_end_time ON drt.drt_my33312_detection_t_det_task (fserial_number, fend_time);
CREATE INDEX idx_det_task_record_task_name ON drt.drt_my33312_detection_t_det_task_record (ftask_id, fdet_sop_task_name);
*/
